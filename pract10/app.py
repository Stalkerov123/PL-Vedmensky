...
def clicked_combobox_calc():
    try:
        if combo.get() == "+":
            result_pole.config(text=float(combo_numbers_1.get()) + float(combo_numbers_2.get()))
        elif combo.get() == "-":
            result_pole.config(text=float(combo_numbers_1.get()) - float(combo_numbers_2.get()))
        elif combo.get() == "*":
            result_pole.config(text=float(combo_numbers_1.get()) * float(combo_numbers_2.get()))
       ...
       showerror(title='ValueError', message='ValueError')

def delete():
    combo_numbers_1.set('')
    combo_numbers_2.set('')
    combo.set('')
    result_pole.config(text='')

root = Tk()
root.geometry('600x350+0+0')
root.title('Ведменский Д.А')
...
btn_c = Button(vkladka_1, text='C', width=84, height=18, bg='grey', command=delete)
btn_c.place(x=1, y=50)
